...
Read `AGENTS.md`, the approved task, relevant design and protocol decisions, task tests, and task-named source files. Confirm the target machine and real build/test/launch commands.
...
- Build the smallest approved native behavior against the existing typed sidecar contract.
...
- Do not change shared-core files unless the task explicitly includes shared-contract ownership.
