...
- **Single source of truth**: state that the repository-configured `github-issues` backend is authoritative. Use one umbrella/control sprint issue per PRD by default. For a genuinely cross-platform PRD, make it the parent control sprint and create linked Windows and macOS implementation sprints; the parent owns combined parity/acceptance, not platform production work. Do not create empty triads for platform-only/docs-only/insufficiently shaped work. Record every split's reason and link, and apply canonical review-size checkpoints.
