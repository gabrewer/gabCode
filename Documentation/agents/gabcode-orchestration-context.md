...
Windows and macOS implementation must be planned as separate platform increments and validated on their respective operating systems. For a PRD that genuinely requires both, create a parent control sprint plus linked Windows and macOS implementation sprints: the parent owns combined scope/acceptance, shared vocabulary/fixtures, dependencies, parity, intentional differences, and child status; each child owns only its complete native implementation and target-OS evidence. Do not create empty triads for platform-only, documentation-only, or insufficiently shaped work. Shared specification or fixture work must be explicit and cannot hide production implementation inside a cross-platform task. A new internal runtime boundary requires a separate human-approved architecture decision.
